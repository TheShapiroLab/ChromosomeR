...
This notebook contains the RNA-seq analysis pipeline. Many heavy steps were not actually executed inside the notebook (the bash and R cells were copied into standalone .sh / .R files and submitted to SLURM on the cluster). Cells that are meant to run in the notebook are marked, and vice versa. The notebook basically exists to keep the whole workflow in one readable narrative.
